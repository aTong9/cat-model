...
- 生产构建通过。
- Special 场景已改为按需加载，富士山、时空传送门和参考特殊场景分别生成独立构建资源；快速切换时只接受最后一次异步结果，卸载时会使待处理加载失效。
- 展台散落装备的创建、射线点击、弹跳物理和资源销毁已抽为独立控制器；布局与交互随机序列使用显式 seed，可复用于后续道路道具。
- 主体四肢已根据 Liberty Cats 动作视频由可见的分段胶囊改为连续蒙皮表面；肩/肘/腕和髋/膝/踝骨骼保留在内部，手掌与脚掌继续作为末端附件，目标是动作时连续、静止时无串珠接缝。
- 主界面动作控制已统一为可配置的站立、岔腿坐、跑步、跳跃四种姿势；点击猫本体会依次切换姿势，点击散落装备仍优先触发装备互动。
- 姿势控制扩展为七种：新增趴下、睡觉和招手；静态姿势与循环动作共用同一套连续蒙皮关节，点击猫会按配置顺序轮换全部姿势。
