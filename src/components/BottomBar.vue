...
async function exportGLB(request = createExportRequest(exportConfig.value)) {
  ...
  try {
    ...
    const budgetWarning = budget.valid ? '' : t('panel.errors.exportBudgetExceeded', { preset: budget.preset, reasons: budget.failures.join(', ') })
    ...
    showNotice('success', [summarizeExportReport(report), budgetWarning].filter(Boolean).join(' · '), 5000)
  } catch (error) {
    console.warn(error)
...
<style scoped>
.bottom-bar{position:fixed;bottom:max(14px,env(safe-area-inset-bottom));left:50%;z-index:100;transform:translateX(-50%);animation:slideUp .5s ease-out .45s both}.bottom-inner{display:flex;align-items:center;gap:6px;padding:6px;border-radius:13px;box-shadow:0 16px 45px rgba(0,0,0,.28)}.bottom-inner .btn{min-height:34px}.random-btn{background:var(--accent);color:#1a1a2e;border-color:var(--accent);font-weight:800}.divider{width:1px;height:22px;background:var(--border);margin:0 2px}.hint-text{padding:0 8px;color:var(--text-dim);font-size:.67rem;white-space:nowrap}.btn:disabled{cursor:wait;opacity:.55}@media(max-width:900px){.bottom-bar{width:calc(100% - 20px);bottom:max(8px,env(safe-area-inset-bottom))}.bottom-inner{justify-content:center;width:100%}.hint-text,.divider{display:none}.bottom-inner .btn{flex:1;min-width:0;padding-inline:6px}}@media(max-width:600px){.bottom-inner{gap:4px;padding:4px}.bottom-inner .btn,.output-menu summary{min-height:46px;padding:4px;font-size:.66rem;white-space:normal;line-height:1.2;text-align:center}}
...
.output-menu{position:relative}.output-menu summary{min-height:34px;list-style:none}.output-menu summary::-webkit-details-marker{display:none}.output-popover{position:absolute;right:0;bottom:calc(100% + 10px);display:grid;grid-template-columns:1fr 1fr;gap:7px;width:350px;max-height:calc(100dvh - 90px);overflow:auto;padding:12px;border-radius:12px;box-shadow:0 18px 48px rgba(0,0,0,.35)}.output-popover header,.output-popover hr,.output-popover .filename{grid-column:1/3}.output-popover header{display:grid;gap:2px}.output-popover header b{color:var(--accent);font-size:.58rem;letter-spacing:.12em}.output-popover header small{color:var(--text-dim);font-size:.54rem}.output-popover label{display:grid;gap:4px;color:#9da6ba;font-size:.57rem}.output-popover select,.output-popover input{min-width:0;padding:7px;border:1px solid var(--border);border-radius:7px;background:#242438;color:var(--text);font-size:.62rem}.output-popover .check{display:flex;align-items:center;gap:6px}.output-popover .check input{accent-color:var(--accent)}.output-popover .btn{justify-content:flex-start;min-height:34px}.output-popover .primary-export{background:var(--accent);color:#201d14;font-weight:800}.output-popover hr{width:100%;border:0;border-top:1px solid var(--border)}@media(max-width:900px){.output-menu{flex:1}.output-menu summary{width:100%}.output-popover{right:0;width:min(380px,calc(100vw - 20px))}}@media(max-width:600px){.output-popover{position:fixed;right:8px;bottom:64px;left:8px;width:auto;grid-template-columns:1fr 1fr;max-height:calc(100dvh - 132px)}}
