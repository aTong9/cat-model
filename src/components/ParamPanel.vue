...
watch(() => store.tokenId, tokenId => { tokenQuery.value = String(tokenId) })
...
const resolvePublicPreview = preview => `${publicBaseUrl}${String(preview).replace(/^\/+/, '')}`
...
const localizedGearList = computed(() => GEAR_LIST.map(option => ({
  ...localizeOption(option),
  preview: resolvePublicPreview(option.preview),
})))
