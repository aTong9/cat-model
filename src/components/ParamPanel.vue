      ...
      <div v-if="store.panelExpanded" id="character-panel" class="panel-body glass">
        ...
        <section v-else-if="activeTab === 'gear'" class="settings-section">
          <SettingBlock :title="t('panel.title.gear')">
            <div class="choice-grid two"><button class="choice" :class="{ active: store.gearType === null }" @click="store.gearType = null">{{ t('panel.noneEquipment') }}</button><button v-for="gear in localizedGearList" :key="gear.id" class="choice" :class="{ active: store.gearType === gear.id }" @click="store.gearType = gear.id">{{ gear.label }}</button></div>
          </SettingBlock>
