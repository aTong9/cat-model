<template>
  <div v-if="store.referenceImage" class="comparison-shell" :class="{ open: store.comparisonOpen }">
    <button class="comparison-toggle glass" aria-controls="comparison-card" :aria-expanded="store.comparisonOpen" @click="store.comparisonOpen = !store.comparisonOpen">
      <span>{{ store.comparisonOpen ? '关闭原图' : '2D / 3D 对照' }}</span><b>{{ store.comparisonOpen ? '×' : '↔' }}</b>
    </button>
    <Transition name="compare">
      <figure v-if="store.comparisonOpen" id="comparison-card" class="comparison-card glass" :class="{ overlay: compareMode === 'overlay' }">
        <header>
