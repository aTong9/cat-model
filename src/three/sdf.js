...
export function sphere({ c, r, k = 0.1, tag = 'body', u = 0, s = null }) {
  if (s) {
    ...
    return {
      ...
      by: c.y,
      bz: c.z,
      br: r * Math.max(s[0], s[1], s[2]) + k,
      dist(x, y, z) {
        const px = (x - c.x) * inv[0],
          py = (y - c.y) * inv[1],
          pz = (z - c.z) * inv[2]
...
export function roundCone({
  ...
  return {
      ...
      u0 === null
        ...
        : (x, y, z) => {
            const t = Math.max(
              ...
              Math.min(
                1,
                ((x - a.x) * bax + (y - a.y) * bay + (z - a.z) * baz) * il2,
              ),
            )
            return u0 + (u1 - u0) * t
          },
    dist(x, y, z) {
      ...
      const z2 = zd * zd * l2
      const kq = srr * x2
      if (Math.sign(zd) * a2 * z2 > kq)
        return Math.sqrt(x2 + z2) * il2 - r2
      if (Math.sign(yd) * a2 * y2 < kq)
...
export function meshFromSDF(prims, cellSize, floorY = 0.004) {
  ...
  // Bucket nearby primitives by small grid blocks
  const blockSize = 8
  ...
  for (let kk = 0; kk < sz; kk++) {
    ...
    for (let j = 0; j < sy; j++) {
      ...
      for (let i = 0; i < sx; i++, idx++) {
        const bucket = buckets[BI(Math.floor(i / blockSize), Math.floor(j / blockSize), Math.floor(kk / blockSize))]
        if (bucket) {
          field[idx] = Math.max(
    ...
    for (let j = 1; j < ny; j++) {
      for (let i = 1; i < nx; i++) {
        ...
        if (i < nx && field[FI(i + 1, j, kk)] < 0 !== inside) {
          quad(
            cellIndex[CI(i, j - 1, kk - 1)],
            cellIndex[CI(i, j, kk - 1)],
            cellIndex[CI(i, j, kk)],
            cellIndex[CI(i, j - 1, kk)],
            inside,
          )
        }
        if (j < ny && field[FI(i, j + 1, kk)] < 0 !== inside) {
          quad(
            cellIndex[CI(i - 1, j, kk - 1)],
            cellIndex[CI(i - 1, j, kk)],
            cellIndex[CI(i, j, kk)],
            cellIndex[CI(i, j, kk - 1)],
            inside,
          )
